"""Data transformation to calculate statistics for absorbance measurement results and visualize them in an HTML report.

DOCUMENTATION FOR THE "i2s_pipacc_data_analysis.py" TRANSFORMATION:

The purpose of this transformation is to analyze data from a pipetting protocol, which is a common procedure in
laboratories where liquid is transferred in precise, measured quantities. The script includes several functions to
calculate and visualize data, such as calculating relative absorbance, converting data formats, comparing dataframe
shapes, setting heatmap layouts, and generating various types of plots (heatmaps and histograms).

INPUTS:
- List of processed file names ("processed_files") to be analyzed
- Corresponding files in the file system
- Run mode ("run_mode") indicating the type of run (quick_run, full_run, eight_run)
- Boolean flag ("do_gravimetric") indicating whether to include gravimetric measurement in the report
- Variables from the Camunda workflow ("variables")

OUTPUTS:
- CSV output saved as files
- HTML report ("end_report") summarizing the results

GENERAL PRINCIPLE:
The script reads the processed files and converts them into pandas DataFrames. It then calculates
the relative absorbance and converts the data into a long format. The script generates several plots, including
heatmaps, histograms, and summary plots, and compiles them into an HTML report. The report includes a table of variables
and their values, histograms of the results, heatmaps of the results, and an overview of the results.

TROUBLESHOOTING:
Main reasons for errors would be issues with the input file format. Further problems could arise if the file names in
the "processed_files" list do not match the file structure.
"""
...
def calculate_weights(weight_well_number: int, weight_expected_well: float, weight_0: float, weight_1: float, weight_2: float, weight_3: float):
    """
    Calculates the weights after each step and returns the input and statistical data.

    This function calculates the weight after each step, computes the mean, standard deviation,
    and coefficient of variation of the weights, and returns a list of two DataFrames.
    The first dataframe contains the input values and the second dataframe contains the statistical results.

    :param weight_well_number: The number of wells in the plate.
    :param weight_expected_well: The expected weight of an empty well.
    :param weight_0: The weight of the empty plate.
    :param weight_1: The weight after step 1.
    :param weight_2: The weight after step 2.
    :param weight_3: The weight after step 3.
    :return: A list of two DataFrames.
    """
    # Calculate the weight after each step
    weights_array = np.array([weight_1 - weight_0, weight_2 - weight_1, weight_3 - weight_2])

    input_df = pd.DataFrame({
        "Input values": ["Number of wells in the plate", "Weight of empty plate (g)",
                         "Weight after step 1 (g)", "Weight after step 2 (g)", "Weight after step 3 (g)",
                         "Plate volume step 1 (ml)", "Plate volume step 2 (ml)", "Plate volume step 3 (ml)"],
        "Value": [weight_well_number, weight_0, weight_1, weight_2, weight_3,
                  weights_array[0], weights_array[1], weights_array[2]]})

    # Calculate the mean of the three weights
    weight_expected_plate = weight_expected_well * weight_well_number / 1000

    weight_mean_plate = np.mean(weights_array)
    weight_mean_well = weight_mean_plate / weight_well_number * 1000

    weight_sd_plate = np.std(weights_array, ddof=1)
    weight_sd_well = weight_sd_plate / weight_well_number * 1000

    weight_cv = weight_sd_plate / weight_mean_plate

    if CV_PERCENTAGE:
        stats_df = pd.DataFrame({
            "Results": ["Expected volume", "Mean of measurements", "SD", "%CV"],
            "Whole plate (ml)": map("{:.2f}".format, [weight_expected_plate, weight_mean_plate, weight_sd_plate, weight_cv*100]),
            "Per well (μl)": map("{:.2f}".format, [weight_expected_well, weight_mean_well, weight_sd_well, weight_cv*100])})
    else:
        stats_df = pd.DataFrame({
            "Results": ["Expected volume", "Mean of measurements", "SD", "CV"],
            "Whole plate (ml)": map("{:.2f}".format, [weight_expected_plate, weight_mean_plate, weight_sd_plate, weight_cv]),
            "Per well (μl)": map("{:.2f}".format, [weight_expected_well, weight_mean_well, weight_sd_well, weight_cv])})

    return [input_df, stats_df]


def generate_assay_table(variables: dict) -> pd.DataFrame:
    """
    Generates a table of assay variables and their values.

    This function takes a dictionary of variables and their values, formats and filters the variables.
    It returns a pandas DataFrame representing a table of the variables and their values.

    :param variables: All variables from the Camunda workflow.
    :return: A DataFrame representing a table of the variables and their values.
    """
    # Define variables for table:
    selected_variables = {
        "Date and time:": f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        "Active user:": variables["active_user"]["value"],
        "Workflow run mode:": variables["run_mode_value"]["value"],
        "Gravimetric measurement:": variables["do_gravimetric"]["value"],
        "Robot type:": variables["robot_type_value"]["value"],
        "Robot ID:": variables["robot_id_value"]["value"],
        "Tested pipette head:": variables["robot_pipette_head_value"]["value"],
        "Dye volume per well (μl):": variables["robot_volume"]["value"],
        "Water volume per well (μl):": variables["robot_water"]["value"],
        "Additional details:": variables["robot_details"]["value"],
        "Uploaded file(s):": '<br>'.join(json.loads(variables["uploaded_files"]["value"])),
        "Processed file(s):": '<br>'.join(json.loads(variables["processed_files"]["value"]))
    }

    df = pd.DataFrame(list(selected_variables.items()), columns=["Variable", "Value"])
...
def run(processed_files: list, run_mode: str, do_gravimetric: bool, variables):
    """
    ...
    :param do_gravimetric: Specifies if gravimetric measurement is undertaken.
    :param variables: Dictionary of all Camunda workflow variables.
    ...
    weight_dfs = []  # empty list to be replaced if gravimetric measurement is done

    # Do gravimetric test:
    if do_gravimetric:
        weight_well_number = int(variables["weight_well_number"]["value"])
        weight_expected = variables["weight_expected"]["value"]
        weight_0 = variables["weight_0"]["value"]
        weight_1 = variables["weight_1"]["value"]
        weight_2 = variables["weight_2"]["value"]
        weight_3 = variables["weight_3"]["value"]

        weight_dfs = calculate_weights(weight_well_number, weight_expected, weight_0, weight_1, weight_2, weight_3)

    # Loop through processed files to create and save everything that is plate-specific:
    for file in processed_files:
        base_name = os.path.splitext(file.replace("processed_", "", 1))[0]
        ...
        rel_df.to_csv(f"data/relative_absorbance_{base_name}.csv")
        rel_df_long.to_csv(f"data/relative_absorbance_long_{base_name}.csv", index=False)
        abs_df.to_csv(f"data/absolute_absorbance_{base_name}.csv")
        abs_df_long.to_csv(f"data/absolute_absorbance_long_{base_name}.csv", index=False)
    ...
    # Add assay table:
    html_list.append(generate_assay_table(variables).to_html(index=False, classes="my-table", escape=False))
    ...
    if run_mode == "full_run":
        ...
        # Add gravimetric tables in case the gravimetric test was done as well:
        if do_gravimetric:
            html_list.append("<h2>Gravimetric measurement</h2>")
            for table in weight_dfs:
                html_list.append(table.to_html(index=False, classes="my-table"))
    ...
    with open("end_report.html", "w") as f:
        f.write(html_doc)
